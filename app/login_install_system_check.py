...
bootstrap=(APP/'startup_bootstrap.py').read_text(encoding='utf-8',errors='ignore') if (APP/'startup_bootstrap.py').exists() else ''
if 'require_blocking_login' not in start and 'require_blocking_login' not in bootstrap: errors.append('Start Center does not require blocking login (require_blocking_login / startup_bootstrap)')
